...
`r if (somalier.run) {"The following subjects were identified as having an inferred sex (via bcftools guess-ploidy plugin) that was different from or not included in the sex linker file provided when running this pipeline.  These subjects may correlate with unexpected genetic duplicates; rarely, there may be evidence of sex chromosome karyotypes that can confound this analysis (e.g. XO, XXY, etc.).  These subjects are not automatically removed from the final VCF and
