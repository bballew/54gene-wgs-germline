...
```{r data_setup, include=TRUE, eval=TRUE}
# set input variable from snakemake
table <- snakemake@input[["benchmarks"]]

# read in tsv
dataset <- read.csv(table, header=TRUE, sep="\t)

# some minor date/time manipulation using lubridate for ease in plotting time
dataset$times <- hms(dataset$h.m.s)
dataset$walltime <- hour(dataset$times) + minute(dataset$times)/60
```
...
exec.time <- ggplot(dataset, aes(x=rule, y=s, colour=process)) +
...
### Read
```{r, echo=FALSE, eval=TRUE, fig.align='center', message=FALSE, warning=FALSE}
# plot the io read
...
### Total CPU time
```{r, cpu_time, eval=TRUE, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE}
# plot the CPU time in hours
# TODO: Ideally we can include boxplots for this set of visualizations
