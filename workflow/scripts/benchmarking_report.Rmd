...
```{r data.setup, include=FALSE, echo=FALSE, eval=TRUE, message=FALSE}
...
```{r load.snakemake.objects, eval=TRUE, echo=FALSE}
in.files <- snakemake@input[["tsv"]]
out.name <- snakemake@params[["benchmarks"]]

# start time to calculate elapsed time
tat.file <- snakemake@input[["start_time"]]

# time cutoff parameter specified in config
time.param <- snakemake@params[["threshold"]]
```

```{r data.prep, eval=TRUE, echo=FALSE, message=FALSE}
# load in the dataset of benchmarking data
dataset <- create.dataset(in.files)

# some light date/time manipulation for ease in plotting
dataset$times <- hms(dataset$`h:m:s`)
d.lub <- hour(dataset$`h:m:s`) + minute(dataset$`h:m:s`)/60 + second(dataset$`h:m:s`)/3600
...
```{r plot.functions, eval=TRUE, echo=FALSE, message=FALSE}
...
```{r render.tsv, eval=TRUE, echo=FALSE, message=FALSE}
# write combined benchmarks file to tsv
create.tsv(dataset, out.name)
```
